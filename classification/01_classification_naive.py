...
if __name__ == "__main__":
    ...
    for e in range(epoch):
        print(f"第{e}轮训练开始")
        batch_size = 10
        # 依次取出10个数据进行训练，譬如0~9， 10~19， 20~29， 等。
        for i in range(0, len(train_dataset_tensors), batch_size):
            # 将当前批次的数据提取出来。[(image_tensor, label), (image_tensor, label), ...]
            datasets_batch = train_dataset_tensors[i:i+batch_size]
            # 从当前批次中提取图片和标签。[(image_tensor, image_tensor,...), (label, label, ...)]
            images_batch, labels_batch = zip(*datasets_batch)
            # # 将图片列表(image_tensor, image_tensor,...), [tensor(3x32x32), tensor(3x32x32), ...]转换为tensor(batch_size, 3, 32, 32)
            images_batch = torch.stack(images_batch)
            # # 将标签列表转换为张量 batch_size
            labels_batch = torch.tensor(labels_batch)
            # print(f"当前批次图片张量形状：{images_batch.shape}, 标签张量形状：{labels_batch.shape}")
            # 训练模型
            # 将图片和标签移动到 GPU 上
        ...
        with torch.no_grad():
            ...
            correct = 0
            total = 0
            for i in range(0, len(test_dataset_tensors), batch_size):
                # 将当前批次的数据提取出来。[(image_tensor, label), (image_tensor, label), ...]
                datasets_batch = test_dataset_tensors[i:i+batch_size]
                # 从当前批次中提取图片和标签。[(image_tensor, image_tensor,...), (label, label, ...)]
                images_batch, labels_batch = zip(*datasets_batch)
                # # 将图片列表(image_tensor, image_tensor,...), [tensor(3x32x32), tensor(3x32x32), ...]转换为tensor(batch_size, 3, 32, 32)
                images_batch = torch.stack(images_batch)
